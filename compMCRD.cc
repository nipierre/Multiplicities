...
void create_kin_plots()
{
  for(int i=0; i<5; i++)
  {
    ...
    fKinematicsRD[i][6] = new TH1F(Form("E_{#mu} %s",trigname[i].c_str()), Form("E_{#mu} %s",trigname[i].c_str()), 100, 0, 160);
    fKinematicsRD[i][7] = new TH1F(Form("E_{#mu'} %s",trigname[i].c_str()), Form("E_{#mu'} %s",trigname[i].c_str()), 100, 0, 160);
    fKinematicsRD[i][8] = new TH1F(Form("#theta %s",trigname[i].c_str()), Form("#theta %s",trigname[i].c_str()), 100, 0, 3);
    fKinematicsRD[i][9] = new TH1F(Form("#phi %s",trigname[i].c_str()), Form("#phi %s",trigname[i].c_str()), 100, 0, 3);
    fKinematicsRD[i][10] = new TH1F(Form("Vertex %s",trigname[i].c_str()), Form("Vertex %s",trigname[i].c_str()), 100, -318.5, -78.5);
  ...
  while(list >> filename)
  {
    ...
    for (Int_t ip = 0; ip < N; ip++)
    {
      ...
      for(int i=0; i<p->GetLeaf("Hadrons.P")->GetLen(); i++)
      {
        ...
        if(0.1<zBj && (LH->GetLeaf("Hadrons.LH")->GetValue(4+6*i)>fLHsec_tab[3])) fKinematicsRD[0][11]->Fill(abs(ph->GetLeaf("Hadrons.ph")->GetValue(i)));
