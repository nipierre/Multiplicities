      ...
      for (int ip = 0; ip < N; ip++)
      {
        ...
        if(fAllDISflag)
        {
          for(int i=0; i<p->GetLeaf("Hadrons.P")->GetLen(); i++)
          {
            ...
            int ydy = y->GetLeaf("y")->GetValue()+p->GetLeaf("Hadrons.P")->GetValue(i)*sin(th->GetLeaf("Hadrons.th")->GetValue(i))*sin(ph->GetLeaf("Hadrons.ph")->GetValue(i))*dz;
            int xdx = x->GetLeaf("x")->GetValue()+p->GetLeaf("Hadrons.P")->GetValue(i)*sin(th->GetLeaf("Hadrons.th")->GetValue(i))*cos(ph->GetLeaf("Hadrons.ph")->GetValue(i))*dz;
