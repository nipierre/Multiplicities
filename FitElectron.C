double FitElectron(double x1, double x2)
{
  ...
  TH1F *h0 = new TH1F();
  Hadron->Draw("(EECAL1+EECAL2)/phad>>h0",Form("(PID==0 || PID==1) && isinECAL==1 && %f<xh && xh<%f",x1,x2));
  h0 = (TH1F*) gDirectory->Get("h0");

  TH1F *h1 = new TH1F();
  Hadron->Draw("(EECAL1+EECAL2)/phad>>h1",Form("(PID==0 || PID==1) && isinECAL==1 && 0.1<(EECAL1_MC+EECAL2_MC)/phad_MC && (EECAL1_MC+EECAL2_MC)/phad_MC<1.25 && %f<xh && xh<%f",x1,x2));
  ...
  double econt = integral/h0->GetEntries();
