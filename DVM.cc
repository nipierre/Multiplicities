  ...
  for(int i=0; i<9; i++)
  {
    for(int j=0; j<6; j++)
    {
      ...
      for(int c=0; c<2; c++)
      {
        for(int k=0; k<12; k++)
        {
          p_d[c][i][j].push_back((1-fDVM_DIS_pi[i][j]+fDVM_DIS_K[i][j] && fDVM_DIS_pi[i][j] && fDVM_DIS_K[i][j]) ? (1-fDVM_h[i][j][k].tab[c][0][0])/(1-fDVM_DIS_pi[i][j]+fDVM_DIS_K[i][j]) : 0);
          k_d[c][i][j].push_back((1-fDVM_DIS_pi[i][j]+fDVM_DIS_K[i][j] && fDVM_DIS_pi[i][j] && fDVM_DIS_K[i][j]) ? (1-fDVM_h[i][j][k].tab[c][0][1])/(1-fDVM_DIS_pi[i][j]+fDVM_DIS_K[i][j]) : 0);
